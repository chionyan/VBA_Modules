...
Function ExtendRange(ByVal argRng As Range, _
    ...
    On Error GoTo errProc
    Set ExtendRange = argRng.Offset(-Top, -Left).Resize(argRng.Rows.Count + Top + Bottom, argRng.Columns.Count + Right + Left)
    Exit Function
errProc:
...
Function FilterRng(ByVal argRng As Range, ByVal args As String, Optional formulaFlag As Long = 0) As Range
    ...
    For Each rng In argRng
        Dim tgtStr As String: tgtStr = rng.Value
        If formulaFlag = 1 Then If rng.HasFormula Then tgtStr = rng.Formula
        If Not IsError(tgtStr) Then
            If re.test(tgtStr) Then
